...
def get_args() -> Args:
    ...
    parser = argparse.ArgumentParser(
        description='Calculate CDS depth ',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    parser.add_argument('fna_file',
                        metavar='bakta fna output',
                        help='The CDS called by annotator in fna')

    parser.add_argument('-g',
                        '--gbk',
                        metavar='gbk file',
                        type=str,
                        help='The gbk formatted file produced by annotator',
                        default='',
                        required=True)
    ...
    parser.add_argument('-d',
                        '--dep',
                        metavar='depth file',
                        help="depth file produced by mapper",
                        type=str,
                        default='',
                        required= True)
    ...
    if not os.path.isfile(args.fna_file):
        parser.error("Fna file does not exist")

    if not os.path.isfile(args.gbk):
        parser.error("Gbk file does not exist")

    if not os.path.isfile(args.dep):
        parser.error("Depth file does not exist")

    return Args(args.fna_file, args.gbk, args.dep)
...
out_CDS_depth = "CDS_depth.tsv"
out_contig_depth = "Contig_depth.tsv"
